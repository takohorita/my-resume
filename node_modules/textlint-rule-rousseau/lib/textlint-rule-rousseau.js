...
var _rousseau = _interopRequireDefault(require("@textlint-rule/rousseau"));
...
module.exports = function textlintRousseau(context) {
  ...
  var createSuggest = replacements => {
    if (replacements.length === 0) {
      return "";
    }

    return "\nSuggestions:\n" + replacements.map(_ref => {
      var {
        value
      } = _ref;
      return "=> " + value;
  ...
  var reportError = (node, source, result) => {
  ...
  return {
    [Syntax.Paragraph](node) {
      ...
      var filteredNode = (0, _unistUtilMap.default)(node, node => {
      ...
      var source = new _textlintUtilToString.StringSource(filteredNode);
      var text = source.toString();

      var reportSourceError = results => {
        reportError(node, source, results);
      };

      (0, _rousseau.default)(text, function (err, results) {
        if (err) {
          throw err;
        }
