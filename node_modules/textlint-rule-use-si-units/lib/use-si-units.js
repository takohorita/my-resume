...
module.exports = (context, userOptions = {}) => {
  ...
  return {
    [Syntax.Str](node) {
      ...
      const matches = (0, _matchIndex.matchCaptureGroupAll)(getSource(node), numberWithUnitTarget);
      matches.forEach(match => {
        ...
        if (parseResult.status === false && isSymbolOnly.status === false) report(node, new RuleError(`「${match.text}」には、SI単位系で使用できない文字が含まれています。SI単位系を使用してください。`, {
          index: match.index
        }));
      });
    }
